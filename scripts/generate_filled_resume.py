...
def build_filled_resume(output_path, font_size_pt=10.5, margin_in=0.45):
    doc = docx.Document()
    ...
    # Bullet Points
    def add_bullet(lead_text, body_text, space_after=2.0):
    ...
    p_sum.paragraph_format.space_after = Pt(4.0)
    ...
    add_section_header("Core Competencies & Technical Skills", space_before=5.0)
    ...
    for cat, desc in comp_data:
        ...
        r_c_lead.bold = True
        r_c_lead.font.size = Pt(font_size_pt - 0.2)
        r_c_lead.font.color.rgb = RGBColor(16, 44, 87)
        
        r_c_body = p_c.add_run(desc)
        r_c_body.font.size = Pt(font_size_pt - 0.2)
        r_c_body.font.color.rgb = RGBColor(40, 40, 40)

    # 4. PROFESSIONAL EXPERIENCE
    add_section_header("Professional Experience", space_before=5.0)

    # Job 1: 1-800 Contacts (Page 1)
    add_job_header("Marketing Strategist / Senior Analyst", "1-800 Contacts", "Draper, UT", space_before=2.5)
    ...
    add_bullet("Paid Media Growth & ROAS Scaling: ", "Directed all paid marketing channels (Google Ads, Bing Ads, Amazon, Facebook, Instagram, TikTok) with a $400k/month budget; scaled monthly revenue 4x from $200k/month to $800k/month while increasing ROAS from 1.5 to 3.5.", space_after=2.0)
    add_bullet("Measurement & Tracking Infrastructure: ", "Architected executive and departmental KPI dashboards in Google Looker Studio; configured custom GA4 event tracking and Google Tag Manager (GTM) containers for granular cross-channel attribution.", space_after=2.0)
    add_bullet("Lifecycle & Conversion Optimization: ", "Revamped email and SMS nurture flows to maximize customer retention and lifetime value; established rigorous landing page conversion rate optimization (CRO) and on-page SEO best practices.", space_after=2.0)
    add_bullet("Cross-Department Planning: ", "Developed a robust multi-department inventory and demand-forecasting model aligning media spend directly with product availability and sales velocity.", space_after=2.0)

    # Job 3: Intercon Inc.
    add_job_header("Director of E-Commerce & Marketing", "Intercon Inc.", "Salt Lake City, UT", space_before=4.0)
    add_bullet("Digital Transformation & Lead Acquisition: ", "Led complete digital marketing and e-commerce transformation for a major B2B distributor, introducing high-volume B2C customer acquisition platforms while maintaining relationships with 650+ retail partners.", space_after=2.0)
    add_bullet("CRM & Lifecycle Database Strategy: ", "Constructed customer database architectures and segmentation frameworks; deployed targeted re-engagement campaigns to reactivate dormant accounts and nurture high-value pipeline leads.", space_after=2.0)
    add_bullet("Multi-Channel Campaign Leadership: ", "Directed multi-platform digital advertising, direct promotional outreach, and print/catalog collateral development utilizing Adobe Creative Suite (Photoshop, InDesign, Illustrator).", space_after=2.0)
    add_bullet("Financial & Reporting Systems: ", "Conducted ongoing cost/benefit and margin analyses to allocate marketing budgets dynamically, building relational databases for accurate cross-department performance tracking.", space_after=2.0)

    # Job 4: The Infinite Agency
    add_job_header("SEM Manager / Project Manager", "The Infinite Agency", "Dallas, TX", space_before=4.0)
    add_bullet("Multi-Account Search Management: ", "Built, launched, and managed 75+ Google Ads accounts with total monthly spend surpassing $276,000, consistently achieving top-tier conversion rates and client ROI benchmarks.", space_after=2.0)
    add_bullet("High-Intent Search & Account Sculpting: ", "Structured high-intent search campaigns with precise keyword match types, negative keyword scrubbing, ad copy A/B testing, and targeted landing page experiences.", space_after=2.0)
    add_bullet("Automated Campaign Monitoring: ", "Programmed custom scripts in JavaScript/Python to automate real-time bid monitoring, budget pacing, and performance anomaly alerts, a system adopted agency-wide.", space_after=2.0)
    add_bullet("Reporting & Multi-Channel Strategy: ", "Built unified client reporting in Funnel.io and Google Data Studio (Looker Studio), counseling clients across SEO, paid search, remarketing, and social media integration.", space_after=2.0)

    # Job 5: LifeSpan Fitness
    add_job_header("E-Commerce & Digital Marketing Manager", "LifeSpan Fitness", "Salt Lake City, UT", space_before=4.0)
    add_bullet("Departmental Leadership & Revenue Impact: ", "Led an 8-person e-commerce and marketing department; managed $400,000+ paid search and advertising budgets generating $9,000,000+ in revenue (35% of total company revenue).", space_after=2.0)
    add_bullet("Full-Funnel Paid Search & CRO: ", "Built and managed 150+ high-converting campaigns across Google Ads, Bing Ads, Gemini, and Amazon; conducted keyword research, custom attribution modeling, and multivariate landing page CRO.", space_after=2.0)
    add_bullet("Lifecycle Marketing & Automation: ", "Built automated customer acquisition and email nurture funnels; developed custom JavaScript automations to optimize promotional campaigns based on real-time business targets.", space_after=2.0)
    add_bullet("Executive Analytics & Reporting: ", "Monitored and reported core KPIs (CPA, ROAS, conversion paths) using Google Analytics, Magento, and QuickBooks to guide senior management forecasting.", space_after=2.0)
    ...
    p_edu.paragraph_format.space_after = Pt(2.5)
    ...
    r_deg_b.bold = True
    r_deg_b.font.size = Pt(font_size_pt - 0.2)
    ...
    r_deg_honors.font.size = Pt(font_size_pt - 0.2)
    ...
    r_lang_b.font.size = Pt(font_size_pt - 0.2)
    ...
    r_cert_h.bold = True
    r_cert_h.font.size = Pt(font_size_pt - 0.2)
    ...
    r_cert_b.font.size = Pt(font_size_pt - 0.2)
...
def render_and_verify():
    ...
    pdf_p = r"c:\Users\DCALL\Desktop\gecko\scratch\preview_filled.pdf"
    
    build_filled_resume(docx_p, font_size_pt=10.5, margin_in=0.45)
    ...
    for i, page in enumerate(pdf):
        ...
        img_path = f"c:\\Users\\DCALL\\Desktop\\gecko\\scratch\\filled_page_{i+1}.png"
        pix.save(img_path)
